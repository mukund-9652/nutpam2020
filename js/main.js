...
$(".event-modal-trigger").click(e => {
  const industryId = $(e.target).attr("event-id");
  if (!industryId)
    return;

  e.preventDefault();
  $(`.event-modal-nav > .active`).removeClass("active");
...
tippy(".event-modal-trigger", {
  sticky: true,
  allowHTML: true,
  theme: "light",
  content: ref => `<p class="event-icon-tooltip m-0">${ref.getAttribute("event-title")}</p>`
});
